...
public class ResulBusquedaPage extends HtmlConstructor {
	 ...
	 * @param valoraciones Lista con las valoraciones promedio de cada juego de la
	 *                     búsqueda
	 ...
	 * @throws SQLException
	 */
	public ResulBusquedaPage(ResultSet rs, ArrayList<Float> valoraciones, String nombre, boolean esAdmin, String foto,
		...
		while (rs.next()) {
			...
			this.contenidoTbody += "<td><a href='Ficha?idJuego=" + rs.getString("juego.id") + "'>"
					+ rs.getString("juego.titulo") + "</a></td>";
		...
		super.setContenidoBody(abreNavBar + contenidoNavBar + cierraNavBar + abreResul + greet + abreTabla + tHead
				+ abreTbody + contenidoTbody + cierraTbody + cierraTabla + cierraResul);
	}
